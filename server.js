...
function sanitizeQuestion(question, index, total, displayMode = 'host-and-player') {
  ...
  const options = Array.isArray(question.options)
    ? question.options.map((opt, idx) => getOptionPayload(opt, idx))
    : [];

  const items = Array.isArray(question.items)
    ? question.items.map((item, idx) => getOrderItemPayload(item, idx))
    : [];
  ...
  const base = {
    ...
    items,
    rows: Math.max(1, Number(question.rows) || 2),
    cols: Math.max(1, Number(question.cols) || 2),
...
function scoreAnswer(question, answer, elapsedMs) {
  ...
  } else if (question.type === 'image-order') {
    const rows = Math.max(1, Number(question.rows) || 2);
    const cols = Math.max(1, Number(question.cols) || 2);
    const total = rows * cols;

    const expected = Array.from({ length: total }, (_, i) => `p${i}`);
    const sent = (Array.isArray(answer.order) ? answer.order : []).map(String);
...
io.on('connection', (socket) => {
  ...
  socket.on('player:submitAnswer', ({ roomCode, answer } = {}, callback = () => {}) => {
    ...
    if (connectedPlayers > 0 && Object.keys(room.currentAnswers).length >= connectedPlayers) {
      endQuestion(code);
    }
  });
